...
class BiliUser:
    ...
    async def send_danmaku(self):
        ...
        if not self.config['DANMAKU_CD']:
            ...
            return
        self.log.log("INFO", "弹幕打卡任务开始....(预计 {} 秒完成)".format(len(self.medals_lower_20) * self.config['DANMAKU_CD']))
        n = 0
        ...
        (await self.api.wear_medal(self.worn_medal['medal']['medal_id'])) if self.config['WEARMEDAL'] and hasattr(self,
                                                                                                                  'worn_medal') else ...
        self.log.log("SUCCESS", "弹幕打卡任务完成")
        self.message.append(f"【{self.name}】 弹幕打卡任务完成 {n}/{len(self.medals_lower_20)}")
